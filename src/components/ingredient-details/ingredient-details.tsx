...
const IngredientDetails = () => {
  ...
  return (
    <div>
      <p className={`text text_type_main-large ${styles.textDetails}`}>Детали ингредиента</p>
      {typeof ingredient === 'undefined' ? (
        <Loader width={100} height={100} className={`${styles.loader} mt-30`} />
      ) : (
        <>
          <div className={styles.image}>
            <img src={ingredient.image_large} alt={ingredient.title} width={480} height={240} />
          </div>

          <p className={`text text_type_main-medium mt-4 ${styles.title}`}>{ingredient.name}</p>
